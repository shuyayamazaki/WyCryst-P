"""
MPVAE model for WyCryst+ Framework
"""
from data import *
...
def MPVAE(X_train, y_train, sup_dim=2, multi=False):
    ...
    if not multi:
        loss_prop = coeff_prop * K.sum(K.square(regression_inputs[:, :1] - y_predict_sup[:, :1])) # single property loss by default 
    else:
        loss_prop = coeff_prop * K.sum(K.square(regression_inputs[:, :sup_dim] - y_predict_sup[:, :sup_dim])) # multi-property loss 
